...
/**
 * version#3: change ArgumentMarshaller to abstract class
 * @author Tosca
 * @date 31/1/2020
 */
...
public class Args3 {
  ...
  public Args3(String schema, String[] args) throws ParseException, ArgsException {
    this.schema = schema;
    this.args = args;
    valid = parse();
  }
  ...
  private void setBooleanArg(char argChar) throws ArgsException {
    ...
    try {
      booleanArgs.get(argChar).set(args[index]);
  ...
  private void setStringArg(char argChar) throws ArgsException {
    int index = currentArgument + 1;
    try {
      stringArgs.get(argChar).set(args[index]);
    } catch (ArrayIndexOutOfBoundsException e) {
      valid = false;
      errorArgumentId = argChar;
  ...
  private void setIntegerArgs(char argChar) throws ArgsException {
    int index = currentArgument + 1;
    try {
      integerArgs.get(argChar).set(args[index]);
    } catch (ArrayIndexOutOfBoundsException e) {
      valid = false;
      errorArgumentId = argChar;
